...
import type { MigrationStatus, MigrationResult, MigrationStatusResponse, MigrationResultResponse } from '../types'
...
export const migrationApi = {
    ...
    async markLegacy(version: number): Promise<{ markedUpTo: number }> {
        const data = await apiFetch<{ result: { markedUpTo: number }; success: boolean }>('/migrations/mark-legacy', {
            method: 'POST',
