 ...
 */
export async function getFreezeStatus(instanceId: string): Promise<FreezeStatus> {
 ...
 */
export async function unfreezeInstance(instanceId: string): Promise<UnfreezeResponse> {
    const response = await fetch(`/api/instances/${instanceId}/freeze`, {
        method: 'DELETE',
    })
    if (!response.ok) {
        const errorData: ErrorResponse = await response.json().catch(() => ({ error: 'Unknown error' })) as ErrorResponse
        throw new Error(errorData.error ?? `Failed to unfreeze: ${String(response.status)}`)
    }
