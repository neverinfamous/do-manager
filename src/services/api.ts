...
export const namespaceApi = {
   ...
   */
  async clone(id: string, newName: string, deepClone?: boolean): Promise<CloneNamespaceResponse> {
    const result = await apiFetch<CloneNamespaceResponse>(`/namespaces/${id}/clone`, {
      method: 'POST',
  ...
  async updateColor(id: string, color: NamespaceColor): Promise<Namespace> {
    const result = await apiFetch<{ namespace: Namespace; success: boolean }>(`/namespaces/${id}/color`, {
      method: 'PUT',
...
export const jobApi = {
  ...
  async list(options?: {
    ...
    if (!skipCache) {
      const cached = getCached(cacheKey, CACHE_TTL.DEFAULT) as Job[] | undefined
      if (cached) {
        return cached
      }
    }
