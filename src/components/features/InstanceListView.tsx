...
export function InstanceListView({
    ...
    const sortedInstances = [...instances].sort((a, b) => {
        ...
        switch (sortField) {
            case 'name':
                comparison = (a.name ?? a.object_id).localeCompare(b.name ?? b.object_id)
                break
            case 'last_accessed':
                {
    ...
    return (
        <div className="overflow-visible border rounded-lg bg-card">
            <table className="w-full text-sm">
                    ...
                    <tr>
                        <th scope="col" className="px-3 py-3 w-10">
                            <Checkbox
                                ...
                                aria-label={allSelected ? 'Deselect all instances' : 'Select all instances'}
                            />
                        </th>
                ...
                <tbody className="divide-y divide-border">
                    {sortedInstances.map((instance) => {
                        ...
                        const storageStatus = instance.storage_size_bytes !== null
                            ? getStorageQuotaStatus(instance.storage_size_bytes)
                            : null
                        const isHighStorage = storageStatus?.level !== 'normal' && storageStatus?.level !== undefined
                        ...
                        return (
                            ...
                            >
                                ...
                                <td className="px-3 py-2">
                                    <div
                                        className={`flex items-center gap-1 ${isCritical
                                            ? 'text-red-500'
                                            : isHighStorage
                                                ? 'text-yellow-500'
            ...
            {instances.length === 0 && (
                <div className="py-8 text-center text-muted-foreground">No instances to display</div>
            )}
        </div>
    )
}
