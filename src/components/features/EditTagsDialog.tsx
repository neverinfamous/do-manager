...
export function EditTagsDialog({
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      const updated = await instanceApi.updateTags(
        instance.id,
        tags,
        namespaceId,
      );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Add tags to <strong>{instance.name ?? instance.object_id}</strong>{" "}
            for organization and search. Tags can be freeform text or{" "}
            <code>key:value</code> style.
          </DialogDescription>
        </DialogHeader>
