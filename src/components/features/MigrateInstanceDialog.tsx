...
export function MigrateInstanceDialog({
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      const migrationResult = await migrateApi.migrateInstance(
        sourceInstance.id,
        {
          targetNamespaceId,
            ...
            <div className="grid gap-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {result.newInstance.name ?? result.newInstance.object_id}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Target Namespace:</span>
                <span className="font-medium">
                  {targetNs?.name ?? "Unknown"}
                </span>
              </div>
              {result.sourceFrozen && (
                <div className="flex justify-between text-sm">
              ...
              <div className="bg-muted rounded-lg p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-sm">
                    Verification{" "}
                    {result.verification.passed ? "Passed" : "Warning"}
                  </span>
                </div>
                <div className="text-xs text-muted-foreground">
                  Source keys: {result.verification.sourceKeyCount} | Target
                  keys: {result.verification.targetKeyCount}
                </div>
              </div>
            )}
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Copy this instance to another namespace. This creates a new instance
            with the same storage data.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          {error && (
          ...
          <div className="grid gap-2">
            ...
            >
              <SelectTrigger id="migrate-target-ns">
                <SelectValue
                  placeholder={
                    namespacesLoading ? "Loading..." : "Select namespace"
                  }
                />
              </SelectTrigger>
              <SelectContent>
            ...
            {namespaces.length === 0 && !namespacesLoading && (
              <p className="text-xs text-muted-foreground">
                No eligible namespaces found. Namespaces must have admin hooks
                enabled.
              </p>
            )}
          </div>
          ...
          <div className="grid gap-2">
            ...
            <div
              className="space-y-2"
              role="radiogroup"
              aria-labelledby="cutover-mode-label"
            >
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setRunVerification(checked === true)
                }
              />
              <Label
                htmlFor="migrate-verify"
            ...
            {sourceHasAlarm && (
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setMigrateAlarms(checked === true)
                  }
                />
                <Label
                  htmlFor="migrate-alarms"
                  className="text-sm font-normal cursor-pointer"
                >
            ...
            {cutoverMode === "copy_delete" && (
              <div className="flex items-center gap-2 bg-destructive/10 p-2 rounded">
                ...
                />
                <Label
                  htmlFor="migrate-confirm-delete"
