...
export function ImportKeysDialog({
  ...
  const handleImport = async (): Promise<void> => {
    ...
    try {
      const result = await importApi.importKeys(
        instanceId,
        parsedData.data,
        mergeMode,
      );
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Import storage keys from a JSON file or paste JSON directly into{" "}
            {instanceName}
          </DialogDescription>
        </DialogHeader>
                ...
                <TabsContent value="file" className="space-y-4 mt-4">
                  ...
                  <div className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">
                      or exported instance storage files
                    </p>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="paste" className="space-y-4 mt-4">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="json-paste-textarea">
                        Paste JSON Data
                      </Label>
                      <div className="flex items-center gap-2">
                          ...
                          >
                            {jsonValidation.isValid ? (
                              <>
                                ...
                                Valid JSON ({jsonValidation.keyCount} key
                                {jsonValidation.keyCount !== 1 ? "s" : ""})
                              </>
                            ) : (
                              <>
                    ...
                    <textarea
                      ...
                      placeholder='{&#10;  "key1": "value1",&#10;  "key2": "value2"&#10;}'
                      className={`w-full min-h-[140px] rounded-md border px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 font-mono resize-y ${
                        pastedJson.trim() && !jsonValidation.isValid
                      ...
                      aria-invalid={
                        pastedJson.trim() ? !jsonValidation.isValid : undefined
                      }
                      disabled={state === "parsing"}
                    />
                    <p
                    ...
                    >
                      Accepts key-value objects or exported instance storage
                      files
                    </p>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg bg-muted p-3 space-y-2">
                ...
                <div className="text-sm text-muted-foreground">
                  <p>
                    <strong>{parsedData.keyCount}</strong> key
                    {parsedData.keyCount !== 1 ? "s" : ""} to import
                  </p>
                  {parsedData.exportedAt && (
                    <p className="text-xs">
                      Exported:{" "}
                      {new Date(parsedData.exportedAt).toLocaleString()}
                    </p>
                  )}
                  {parsedData.sourceInstance?.name && (
                    <p className="text-xs">
                      Source: {parsedData.sourceInstance.name}
                    </p>
                  )}
                </div>
              </div>
        ...
        <DialogFooter>
          ...
          {state === "success" && <Button onClick={handleDone}>Done</Button>}
