...
export const MIGRATIONS: Migration[] = [
    {
        version: 1,
        name: 'initial_schema',
        description: 'Base schema with namespaces, instances, jobs, audit_log, backups',
        sql: `
      -- Tracked namespaces
    ...
    },
    {
        ...
        description: 'Add color column to namespaces table for visual organization (phase9)',
        sql: `
    ...
    },
    {
        version: 6,
        name: 'instance_tags',
        description: 'Add tags column to instances table for tagging and search (phase10)',
        sql: `
    ...
    try {
        ...
        for (const migration of pendingMigrations) {
            try {
                ...
                logWarning(`Failed to apply migration ${migration.version}: ${errorMessage}`, {
                    module: 'migrations',
                    operation: 'apply_single',
...
export async function detectLegacyInstallation(db: D1Database): Promise<LegacyInstallationInfo> {
    try {
        ...
        if (existingTables.includes('namespaces') && existingTables.includes('instances')) {
            suggestedVersion = 1
        }
