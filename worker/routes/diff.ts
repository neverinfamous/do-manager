...
async function fetchInstanceStorage(
  ...
  if (!namespace.endpoint_url || namespace.admin_hook_enabled !== 1) {
    throw new Error(`Admin hook not configured for namespace ${namespace.name}`)
  }
  ...
  if (!listResponse.ok) {
    throw new Error(`Failed to list keys for ${instanceName}: ${String(listResponse.status)}`)
  }
...
async function compareInstances(
  ...
  if (body.instanceIdA === body.instanceIdB) {
    return errorResponse('Cannot compare an instance with itself', corsHeaders, 400)
  }
  ...
  try {
    ...
    ).bind(body.instanceIdA).first<Instance>()
    ...
    const instanceB = await env.METADATA.prepare(
      'SELECT * FROM instances WHERE id = ?'
    ).bind(body.instanceIdB).first<Instance>()
    ...
    ).bind(instanceA.namespace_id).first<Namespace>()

    const namespaceB = await env.METADATA.prepare(
      'SELECT * FROM namespaces WHERE id = ?'
    ).bind(instanceB.namespace_id).first<Namespace>()
...
function computeDiff(
  ...
  instanceInfoA: { id: string; name: string; namespaceId: string; namespaceName: string },
  instanceInfoB: { id: string; name: string; namespaceId: string; namespaceName: string }
): DiffResult {
