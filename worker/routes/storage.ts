 ...
 */
const MOCK_STORAGE: Record<
  string,
  { keys: Record<string, unknown>; tables: string[] }
> = {
  "inst-1": {
    keys: {
      ...
      return jsonResponse(
        {
          ...
          tables: [],
          warning:
            "Admin hook endpoint not configured. Go to namespace settings to configure the endpoint URL pointing to your Worker with admin hooks enabled.",
      ...
      return jsonResponse(
        {
          ...
          tables: [],
          warning:
            "Admin hook is not enabled for this namespace. Enable it in namespace settings after adding admin hook methods to your DO class.",
          admin_hook_required: true,
        },
        corsHeaders,
      );
  ...
  try {
    ...
    if (!namespace?.endpoint_url || namespace.admin_hook_enabled !== 1) {
      return errorResponse(
        "Admin hook not configured or enabled",
        corsHeaders,
        400,
      );
    }
    ...
    if (!response.ok) {
      return errorResponse(
        `Admin hook error: ${String(response.status)}`,
        corsHeaders,
        response.status,
      );
    }
...
async function executeSql(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => "Unknown error");
      return errorResponse(
        `SQL execution failed: ${errorText}`,
        corsHeaders,
        response.status,
      );
    }
...
async function importStorage(
  ...
  if (keyCount === 0) {
    return errorResponse(
      "data object must contain at least one key",
      corsHeaders,
      400,
    );
  }
  ...
  try {
    ...
    logInfo(
      `Calling admin hook import: ${adminUrl} keys: ${String(keyCount)}`,
      {
        module: "storage",
        operation: "import",
        instanceId,
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => "Unknown error");
      logWarning(
        `Import admin hook error: ${String(response.status)} ${errorText}`,
        {
          module: "storage",
          operation: "import",
          instanceId,
      ...
      await failJob(
        env.METADATA,
        jobId,
        `Admin hook error: ${String(response.status)} - ${errorText.slice(0, 100)}`,
      );
      return errorResponse(
        `Admin hook error: ${String(response.status)}`,
        corsHeaders,
...
async function renameStorageKey(
  ...
  if (oldKey === newKey) {
    return errorResponse(
      "newKey must be different from oldKey",
      corsHeaders,
      400,
    );
  }
  ...
  try {
    ...
    if (!getResponse.ok) {
      await failJob(
        env.METADATA,
        jobId,
        `Failed to get old key: ${String(getResponse.status)}`,
      );
      return errorResponse(
        `Failed to get old key: ${String(getResponse.status)}`,
        corsHeaders,
    ...
    if (!putResponse.ok) {
      await failJob(
        env.METADATA,
        jobId,
        `Failed to create new key: ${String(putResponse.status)}`,
      );
      return errorResponse(
        `Failed to create new key: ${String(putResponse.status)}`,
        corsHeaders,
    ...
    if (!deleteResponse.ok) {
      ...
      logWarning(
        `Failed to delete old key during rename: ${String(deleteResponse.status)}`,
        {
          module: "storage",
    ...
    logWarning(
      `Rename key error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "storage",
        operation: "rename_key",
        instanceId,
    ...
    return errorResponse(
      `Failed to rename key: ${error instanceof Error ? error.message : "Unknown error"}`,
      corsHeaders,
      500,
    );
  }
...
async function getFreezeStatus(
  ...
  try {
    ...
    if (!namespace?.endpoint_url || namespace.admin_hook_enabled !== 1) {
      return jsonResponse(
        { frozen: false, warning: "Admin hook not configured" },
        corsHeaders,
      );
    }
    ...
    logWarning(
      `Get freeze status error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "storage",
...
async function unfreezeInstance(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return errorResponse(
        `Failed to unfreeze: ${errorText.slice(0, 100)}`,
        corsHeaders,
        response.status,
      );
    }
    ...
    logWarning(
      `Unfreeze error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "storage",
        operation: "unfreeze",
        instanceId,
    ...
    return errorResponse(
      `Failed to unfreeze: ${error instanceof Error ? error.message : "Unknown error"}`,
      corsHeaders,
      500,
    );
  }
