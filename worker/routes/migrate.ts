...
import type { Env, CorsHeaders, Instance, Namespace, ErrorContext } from '../types'
...
export async function handleMigrateRoutes(
    ...
    if (method === 'POST' && migrateMatch) {
        ...
        return migrateInstance(request, instanceId, env, corsHeaders, isLocalDev, userEmail)
    }
...
async function migrateInstance(
    ...
    if (!['copy', 'copy_freeze', 'copy_delete'].includes(cutoverMode)) {
        return errorResponse('Invalid cutoverMode. Must be: copy, copy_freeze, or copy_delete', corsHeaders, 400)
    }

    logInfo(`Starting migration for instance ${sourceInstanceId}`, createContext('migrate_start', userEmail, {
    ...
    if (isLocalDev) {
        const mockResponse: MigrateInstanceResponse = {
            ...
            ...(runVerification ? { verification: { passed: true, sourceKeyCount: 10, targetKeyCount: 10 } } : {}),
            warnings: []
        }
    ...
    // Create job record
    const jobId = await createJob(env.METADATA, 'migrate_instance', userEmail, null, sourceInstanceId)
    ...
    try {
        ...
        if (!sourceNamespace?.endpoint_url) {
            await failJob(env.METADATA, jobId, 'Source namespace endpoint not configured')
            return errorResponse('Source namespace endpoint not configured. Set up admin hook first.', corsHeaders, 400)
        }

        // Get target namespace
        const targetNamespace = await env.METADATA.prepare(
            'SELECT * FROM namespaces WHERE id = ?'
        ).bind(body.targetNamespaceId).first<Namespace>()
        ...
        if (!targetNamespace.endpoint_url) {
            await failJob(env.METADATA, jobId, 'Target namespace endpoint not configured')
            return errorResponse('Target namespace endpoint not configured. Set up admin hook first.', corsHeaders, 400)
        }

        // Prevent migrating to same namespace
        if (sourceNamespace.id === targetNamespace.id) {
            await failJob(env.METADATA, jobId, 'Cannot migrate to the same namespace')
            return errorResponse('Cannot migrate to the same namespace. Use clone instead.', corsHeaders, 400)
        }

        // Determine target instance name
        const finalTargetName = targetInstanceName ?? sourceInstance.name ?? sourceInstance.object_id
        ...
        ).bind(body.targetNamespaceId, finalTargetName, finalTargetName).first<{ id: string }>()

        if (existingTarget) {
            await failJob(env.METADATA, jobId, 'Instance with this name already exists in target namespace')
            return errorResponse('Instance with this name already exists in target namespace', corsHeaders, 409)
        }
        ...
        logInfo('Exporting from source instance', createContext('EXPORT', userEmail))
        ...
        if (!exportResponse.ok) {
            ...
            return errorResponse(`Failed to export source instance: ${errorText.slice(0, 100)}`, corsHeaders, exportResponse.status)
        }

        interface ExportData { data: Record<string, unknown>; keyCount?: number }
        const exportData: ExportData = await exportResponse.json()
        const sourceKeyCount = exportData.keyCount ?? Object.keys(exportData.data ?? {}).length
        logInfo(`Export returned ${sourceKeyCount} keys`, createContext('export_result', userEmail, { keyCount: sourceKeyCount }))
        ...
        if (!importResponse.ok) {
            ...
            return errorResponse(`Failed to import to target instance: ${errorText.slice(0, 100)}`, corsHeaders, importResponse.status)
        }
        ...
        if (migrateAlarms && sourceInstance.has_alarm) {
            try {
                ...
                if (alarmResponse.ok) {
                    const alarmData = await alarmResponse.json() as { alarm: number | null }
        ...
        if (runVerification) {
            try {
                ...
                if (verifyResponse.ok) {
                    ...
                    const targetKeyCount = verifyData.keyCount ?? Object.keys(verifyData.data).length
                    verification = {
                    ...
                    if (!verification.passed) {
                        warnings.push(`Key count mismatch: source=${sourceKeyCount}, target=${targetKeyCount}`)
                    }
                }
            } catch {
        ...
        if (cutoverMode === 'copy_freeze') {
            ...
            logInfo(`Freezing source instance: ${freezeUrl}`, createContext('FREEZE', userEmail))
            try {
                ...
                if (!freezeResponse.ok) {
                    ...
                    logInfo(`Freeze failed: ${freezeResponse.status} - ${errorText}`, createContext('FREEZE_FAILED', userEmail))
                    warnings.push('Failed to freeze source instance')
                } else {
                    logInfo('Source instance frozen successfully', createContext('FREEZE_SUCCESS', userEmail))
                }
            } catch (error) {
                logInfo(`Freeze error: ${error instanceof Error ? error.message : String(error)}`, createContext('FREEZE_ERROR', userEmail))
                warnings.push('Failed to freeze source instance')
            }
    ...
    `).bind(
        ...
        logInfo(`Migration completed for instance ${sourceInstanceId}`, createContext('migrate_complete', userEmail, {
            newInstanceId,
            cutoverMode,
            sourceFrozen,
            sourceDeleted
        ...
        void logError(env, `Migration failed: ${errorMessage}`, createContext('migrate_error', userEmail), isLocalDev)
