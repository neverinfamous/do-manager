...
async function listInstances(
  ...
  if (isLocalDev) {
    const filtered = MOCK_INSTANCES.filter(
      (i) => i.namespace_id === namespaceId,
    );
    return jsonResponse(
      {
...
async function createInstance(
  ...
  try {
    ...
    const existing = await env.METADATA.prepare(
      ...
      .bind(namespaceId, body.object_id)
      .first<Instance>();
    ...
    if (existing) {
      ...
      await completeJob(env.METADATA, jobId, {
        ...
        name: existing.name ?? existing.object_id,
    ...
    `,
      ...
      .bind(id, namespaceId, body.name ?? null, body.object_id, now, now, now)
      .run();
    ...
    if (jobId) {
      await env.METADATA.prepare("UPDATE jobs SET instance_id = ? WHERE id = ?")
        .bind(id, jobId)
        .run();
    }
    await completeJob(env.METADATA, jobId, {
      instance_id: id,
      name: body.name ?? body.object_id,
    });
...
async function deleteInstance(
  ...
  try {
    ...
    const inst = await env.METADATA.prepare(
      "SELECT * FROM instances WHERE id = ?",
    )
      .bind(instanceId)
      .first<Instance>();
...
async function updateInstanceAccessed(
  ...
  try {
    ...
    logWarning(
      `Update accessed error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "instances",
...
async function cloneInstance(
  ...
  if (isLocalDev) {
    ...
    const sourceInstance = MOCK_INSTANCES.find(
      (i) => i.id === sourceInstanceId,
    );
    if (!sourceInstance) {
  ...
  try {
    ...
    await env.METADATA.prepare(
      ...
      .bind(sourceInstance.namespace_id, sourceInstanceId, jobId)
      .run();
    ...
    const existing = await env.METADATA.prepare(
      ...
      .bind(sourceInstance.namespace_id, newName, newName)
      .first<{ id: string }>();
    ...
    `,
      ...
      .bind(newId, sourceInstance.namespace_id, newName, newName, now, now, now)
      .run();
...
async function updateInstanceColor(
  ...
  try {
    ...
    `,
      ...
      .bind(color ?? null, now, instanceId)
      .run();
    ...
    logWarning(
      `Update color error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "instances",
...
async function renameInstance(
  ...
  try {
    ...
    await env.METADATA.prepare(
      ...
      .bind(existing.namespace_id, instanceId, jobId)
      .run();
    ...
    await failJob(
      env.METADATA,
      jobId,
      error instanceof Error ? error.message : "Failed to rename instance",
    );
...
async function updateInstanceTags(
  ...
  try {
    ...
    logWarning(
      `Update tags error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "tags",
        operation: "update",
        instanceId,
