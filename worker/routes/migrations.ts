...
export async function handleMigrationRoutes(
    ...
    if (request.method === 'GET' && url.pathname === '/api/migrations/status') {
        ...
        if (isLocalDev) {
            const mockStatus = {
                ...
                appliedMigrations: [
                    ...
                    { version: 2, migration_name: 'webhooks', applied_at: new Date().toISOString() },
                    { version: 3, migration_name: 'alarm_history', applied_at: new Date().toISOString() },
                    { version: 4, migration_name: 'saved_queries_and_colors', applied_at: new Date().toISOString() }
                ],
                isUpToDate: true
            }

            return new Response(JSON.stringify({ result: mockStatus, success: true }), {
        ...
        try {
            ...
            logWarning(`Failed to get migration status: ${errorMessage}`, createContext('status', userEmail))
    ...
    if (request.method === 'POST' && url.pathname === '/api/migrations/apply') {
        ...
        if (isLocalDev) {
            ...
            return new Response(JSON.stringify({ result: mockResult, success: true }), {
                headers: jsonHeaders(corsHeaders)
            })
        }
        ...
        try {
            ...
            if (result.success) {
                logInfo(`Successfully applied ${result.migrationsApplied} migration(s)`, createContext('apply', userEmail, {
                    migrationsApplied: result.migrationsApplied,
                    currentVersion: result.currentVersion
                }))
            } else {
                void logError(env, `Migration failed: ${result.errors.join(', ')}`, createContext('apply', userEmail, { errors: result.errors }), isLocalDev)
            }
            ...
            const errorMessage = err instanceof Error ? err.message : String(err)
            void logError(env, `Failed to apply migrations: ${errorMessage}`, createContext('apply', userEmail), isLocalDev)
    ...
    if (request.method === 'POST' && url.pathname === '/api/migrations/mark-legacy') {
        logInfo('Marking legacy migrations', createContext('mark_legacy', userEmail))

        if (isLocalDev) {
            return new Response(JSON.stringify({ result: { markedUpTo: 4 }, success: true }), {
                headers: jsonHeaders(corsHeaders)
            })
        }
        ...
        try {
            ...
            logInfo(`Marked migrations up to version ${targetVersion} as applied`, createContext('mark_legacy', userEmail, { version: targetVersion }))

            return new Response(JSON.stringify({ result: { markedUpTo: targetVersion }, success: true }), {
                headers: jsonHeaders(corsHeaders)
            })
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : String(err)
            logWarning(`Failed to mark legacy migrations: ${errorMessage}`, createContext('mark_legacy', userEmail))
