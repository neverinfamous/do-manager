...
export async function detectCompletedAlarms(env: Env): Promise<void> {
  try {
    ...
    if (expiredAlarms.results.length > 0) {
      logInfo(
        `Marked ${String(expiredAlarms.results.length)} alarm(s) as completed`,
        {
          module: "health",
    ...
    logWarning(
      `Failed to detect completed alarms: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "health",
        operation: "detect_completed_alarms",
        metadata: {
...
async function getHealthSummary(
  ...
  try {
    ...
    const warningThresholdBytes = Math.floor(
      STORAGE_QUOTA.MAX_BYTES * STORAGE_QUOTA.WARNING_THRESHOLD,
    );
    const criticalThresholdBytes = Math.floor(
      STORAGE_QUOTA.MAX_BYTES * STORAGE_QUOTA.CRITICAL_THRESHOLD,
    );
    ...
    const health: HealthSummary = {
      ...
      highStorageInstances: highStorageResult.results.map((row) => {
        const percentUsed = Math.round(
          (row.storageSizeBytes / STORAGE_QUOTA.MAX_BYTES) * 100,
        );
        return {
          id: row.id,
          name: row.name,
          ...
          percentUsed,
          level:
            row.storageSizeBytes >= criticalThresholdBytes
              ? ("critical" as const)
              : ("warning" as const),
        };
      }),
    ...
    logWarning(
      `Get summary error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "health",
