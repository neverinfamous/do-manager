...
const MOCK_SAVED_QUERIES: SavedQuery[] = [
  ...
  },
  {
    ...
    query:
      "SELECT user_id, COUNT(*) as session_count FROM sessions WHERE active = 1 GROUP BY user_id",
...
async function listQueries(
  ...
  if (isLocalDev) {
    const filtered = MOCK_SAVED_QUERIES.filter(
      (q) => q.namespace_id === namespaceId,
    );
...
async function createQuery(
  ...
  try {
    ...
    `,
      ...
      .bind(id, namespaceId, name, description, query, now, now)
      .run();
...
async function deleteQuery(
  ...
  try {
    await env.METADATA.prepare("DELETE FROM saved_queries WHERE id = ?")
      .bind(queryId)
      .run();
