...
async function setAlarm(
  ...
  if (body.timestamp < Date.now()) {
    return errorResponse(
      "Alarm timestamp must be in the future",
      corsHeaders,
      400,
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      await failJob(
        env.METADATA,
        jobId,
        `Failed to set alarm: ${String(response.status)}`,
      );
      return errorResponse(
        "Failed to set alarm on DO",
        corsHeaders,
        response.status,
      );
    }
    ...
    await env.METADATA.prepare(
      ...
      .bind("cancelled", now, instanceId, "scheduled")
      .run();
  ...
  try {
    ...
    if (!response.ok) {
      await failJob(
        env.METADATA,
        jobId,
        `Failed to delete alarm: ${String(response.status)}`,
      );
      return errorResponse(
        "Failed to delete alarm on DO",
        corsHeaders,
        response.status,
      );
    }
