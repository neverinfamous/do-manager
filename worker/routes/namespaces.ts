...
function isSystemNamespace(name: string): boolean {
  return SYSTEM_DO_PATTERNS.some(
...
async function discoverNamespaces(
  ...
  try {
    ...
    logWarning(
      `Discover error: ${error instanceof Error ? error.message : String(error)}`,
      {
        module: "namespaces",
        operation: "discover",
        metadata: {
...
async function addNamespace(
  ...
  try {
    ...
    await failJob(
      env.METADATA,
      jobId,
      error instanceof Error ? error.message : "Failed to add namespace",
    );
...
async function updateNamespace(
  ...
  if (isLocalDev) {
    ...
    const updated: Namespace = {
      ...
      endpoint_url:
        body.endpoint_url !== undefined ? body.endpoint_url : ns.endpoint_url,
...
async function deleteNamespace(
  ...
  try {
    ...
    const ns = await env.METADATA.prepare(
...
async function cloneNamespace(
  ...
  try {
    ...
    if (
      deepClone &&
      (!sourceNs.endpoint_url || sourceNs.admin_hook_enabled !== 1)
    ) {
      await failJob(
        env.METADATA,
        jobId,
    ...
    if (deepClone && sourceNs.endpoint_url) {
        ...
        try {
          ...
          if (!exportResponse.ok) {
            const errorText = await exportResponse
              ...
              .catch(() => "Unknown error");
            warnings.push(
              `Failed to export instance "${instanceName}": ${errorText.slice(0, 50)}`,
            );
          ...
          if (!importResponse.ok) {
            const errorText = await importResponse
              ...
              .catch(() => "Unknown error");
            warnings.push(
              `Failed to import to instance "${newInstanceName}": ${errorText.slice(0, 50)}`,
            );
            failedInstances.push(instanceName);
            continue;
          }
          ...
          warnings.push(
            `Error cloning instance "${instanceName}": ${err instanceof Error ? err.message : String(err)}`,
          );
          failedInstances.push(instanceName);
        }
      }
      ...
      if (failedInstances.length > 0) {
        warnings.unshift(
          `${String(failedInstances.length)} of ${String(instances.length)} instances had issues during cloning`,
        );
      }
    }
...
async function updateNamespaceColor(
  ...
  try {
    ...
    `,
      ...
      .bind(color ?? null, now, namespaceId)
      .run();
